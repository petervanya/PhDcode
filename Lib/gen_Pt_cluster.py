#!/usr/bin/env python
"""Usage:
    gen_Pt_cluster.py water [-s <s>] [-p <p>] [-r <r>] [--save]
    gen_Pt_cluster.py Pt <cluster> [-s <s>] [--save <fname>]
    gen_Pt_cluster.py carbon <Nh> <Nv> [--addH [--Ha <Ha>]] [-s <s>] [--save]
...
def gen_Pt(a=2.775, cluster="3", shift=np.zeros(3), shiftPt=np.zeros(3),\
    ...
    * three-layer: 5_10_5, 9_10_9
    ...
    # three layers
        ...
        sys.exit("Cluster not implemented, choose a different one.")
    return mol
...
def gen_carbon(Nh, Nv, a=1.42, shift=np.zeros(3)):
    ...
    for n in range(1, N):
      	j = n % Nv
      	i = n / Nv #(n-j)/Nv
      	base = np.array([i*dx, j*dy + i*dy/2, 0.0])
...
if __name__ == "__main__":
    ...
    if args["water"]:
        ...
        print(mol)
        if args["--save"]:
            mol.save("water.xyz")    # "water.xyz"
    elif args["Pt"]:
        cluster = args["<cluster>"]
        mol = gen_Pt(cluster=cluster, shift=shift)
        print(mol)
        if args["--save"]:
            fname = "Pt_" + cluster + ".xyz"
            mol.save(fname)    # "Pt.xyz"
    elif args["carbon"]:
        a = 1.42
