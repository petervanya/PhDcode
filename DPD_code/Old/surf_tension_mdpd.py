...
def density(xyz, Ls, rd=0.75, rc=1.0):
...
def add_images(nm, xyz, rho, Ls, u, rc=1.0):
    """Transfer images from one end of the box to the other,
    along a coord u in [0, 1, 2]"""
    xyz_low = xyz[xyz[:, u] < rc]
    xyz_low[:, u] += Ls[u]
    ...
    rho_low = rho[xyz[:, u] < rc]
    rho_high = rho[xyz[:, u] > Ls[u] - rc]
    return np.r_[nm, nm_low, nm_high], np.r_[xyz, xyz_low, xyz_high], \
            np.r_[rho, rho_low, rho_high]


def pressure_vec(nm, xyz, rho, cA, cB, box, u, Nbins, rd=0.75, rc=1.0):
    """
    ...
    for nb in range(Nbins):
        ...
        for ni in range(Nl):
            for nj in range(ni):
                ...
                P[nb, :] += drn * F * ksi(us[nb], xyz[i, u], xyz[j, u])
    return P / Area


def surf_tension_kb(nm, xyz, rho, cA, cB, box, u, Nbins, rd=0.75, rc=1.0):
...
def surf_tension_kb_lc(nm, xyz, rho, cA, cB, box, u, Nbins, rd=0.75, rc=1.0):
    """Kirkwood-Buff version of computing surface tension"""
    N = len(xyz)
    Ls = np.diag(box)
    V = np.prod(Ls)
    ...
    for cell in lc.cells.values():
        ...
        for i in range(Na):
            for j in range(i):
                ...
                A = cA[nm[cell[i]], nm[cell[j]]]
                B = cB[nm[cell[i]], nm[cell[j]]]
                nr = norm_numba(drn)
...
if __name__ == "__main__":
    ...
    if args["ik"]:
        ...
        for frame in frames:
            ...
            nm = nm.astype(int) - 1
            rho = density(xyz, Ls, rd, rc=1.0)
            nm, xyz, rho = add_images(nm, xyz, rho, Ls, u)
            Pi = pressure_vec(nm, xyz, rho, coeffsA, coeffsB, \
    ...
    if args["kb"]:
        ...
        for frame in frames:
            ...
            nm = nm.astype(int) - 1
            rho = density(xyz, Ls, rd, rc=1.0)
            gammai = surf_tension_kb(nm, xyz, rho, coeffsA, coeffsB, \
                    box, u, Nbins, rd, rc=1.0)
            gamma += gammai / Nf
            print("Gamma:", gammai)
        tf = time.time()
