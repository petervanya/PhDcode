...
def gen_inter_coeffs(atoms_yaml, bead_types, a_DPD, b_DPD, gamma=4.5, rc=1.0):
    """
    ...
    * bead_types, e.g. "ABCW"
    * chi2da = 3.27 from Groot-Warren, JCP, 1997 for rho = 3
      or 0.606 from Ghoufi, JCTC, 2012
    """
    Nbt = len(bead_types)
    a_ij = {}
    for i in range(Nbt):
...
if __name__ == "__main__":
    ...
    a_ij = gen_inter_coeffs(data["chi-params"], bead_types, \
            a_DPD, b_DPD, gamma, rc=1.0)
    masses = dict( (i, 1.0) for i in range(1, Nbt+1) )

    # ==== printing
    dlms.save_config("CONFIG", atom_ids_l, xyz, np.eye(L))
