#!/usr/bin/env python
"""Usage:
    gen_yamam.py <input> (dlms | lammps) [--xyz]
...
rho_DPD = 3.0
a_DPD = 104.0
r_DPD = 7.1e-10               # DPD distance unit
k0 = 4.0
...
def calc_nc_nw(N, Nmc, Nbm, lmbda):
    ...
    Nw = int(lmbda / 4 / (lmbda / 4 + Nbm) * N)
    Nc = int(N / (Nmc * (lmbda / 4 + Nbm)))
    return Nc, Nw
...
if __name__ == "__main__":
    ...
    print("tau: %.2e | Box size: %.1f | a_DPD: %0.1f | lmbda: %i" % \
            (tau, L, a_DPD, lmbda))
